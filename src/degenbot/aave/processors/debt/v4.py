...
class DebtV4Processor(DebtV1Processor):
    ...
    def calculate_scaled_amount(self, raw_amount: int, index: int) -> int:
        ...
        Uses floor division (ray_div_floor) to match revision 4 vToken
    ...
    def calculate_mint_scaled_amount(self, raw_amount: int, index: int) -> int:
        ...
        Uses ceiling division (ray_div_ceil) to match revision 4 vToken
    ...
    def process_burn_event(
        ...
        Revision 4 uses floor division (ray_div_floor) to match
    ...
    def process_mint_event(
        ...
        Revision 4 uses ceiling division (ray_div_ceil) for BORROW operations
        to match TokenMath.getVTokenMintScaledAmount behavior.

        For V4+, the scaled amount for BORROW operations must be calculated
        ...
        Args:
            ...
            scaled_delta: Pre-calculated scaled amount from original borrow amount.
                Must be provided for accurate BORROW processing in V4+.
