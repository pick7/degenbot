...
class TransactionContext:
    ...
    # Track Mint events that have been matched to BalanceTransfer events.
    # Key: Mint event logIndex, Value: True if matched to a BalanceTransfer
    # This prevents double-counting when interest accrual Mint events are followed
...
def _log_token_operation(
    *,
    ...
    # Also log to structured debug logger if enabled
    if aave_debug_logger.is_enabled():
        # Parse balance_info (format: "before -> after")
        balance_before = 0
        balance_after = 0
        if " -> " in balance_info:
            parts = balance_info.split(" -> ")
            try:
                balance_before = int(parts[0])
                balance_after = int(parts[1])
            except ValueError:
                pass

        position_type = "collateral" if token_type == "aToken" else "debt"
        block_number = int(block_info.split(".")[0]) if "." in block_info else 0

        aave_debug_logger.log_position_update(
            user_address=user_address,
            position_type=position_type,
            token_address=token_address,
            block_number=block_number,
            tx_hash=tx_hash,
            operation=user_operation.value,
...
def _process_collateral_burn_with_match(
    ...
    if aave_debug_logger.is_enabled() and context.operation is not None:
        if context.operation.operation_type in {
            OperationType.LIQUIDATION,
            OperationType.GHO_LIQUIDATION,
            OperationType.SELF_LIQUIDATION,
        }:
            ...
            aave_debug_logger.log_liquidation_match(
                ...
                block_number=scaled_event.event["blockNumber"],
                tx_hash=context.tx_context.tx_hash,
            )
...
def _process_staked_aave_event(
    ...
    *,
    ...
    match discount_token_info_event["topics"][0]:
        case AaveV3StkAaveEvent.STAKED.value:
        ...
        case AaveV3StkAaveEvent.REDEEM.value:
        ...
        case ERC20Event.TRANSFER.value:
...
def _process_collateral_mint_event(
    *,
    ...
    if caller_address == pool_contract.address:
        ...
        if event_amount > balance_increase:
            ...
            result = matcher.find_matching_pool_event(
                ...
                reserve_address=reserve_address,
                check_users=[user.address],
                try_event_type_first=AaveV3PoolEvent.WITHDRAW,  # Try WITHDRAW before SUPPLY
            )
    ...
    if (
        ...
        and matched_pool_event["topics"][0] == AaveV3PoolEvent.SUPPLY.value
    ):
    ...
    *,
    ...
    if event_amount != balance_increase:
        ...
        if (
            ...
            and result["pool_event"]["topics"][0] == AaveV3PoolEvent.BORROW.value
        ):
...
def _process_collateral_burn_event(
    *,
    ...
    """Process a collateral (aToken) burn event using EventMatcher.

    Uses EventMatcher to find matching Pool events (WITHDRAW, REPAY, or LIQUIDATION_CALL)
    and extracts the appropriate scaled amount based on the matched event type.
    ...
    # Use EventMatcher to find matching pool event
    # EventMatcher handles LIQUIDATION_CALL reuse (Bug #0009, #0010)
    # and REPAY conditional consumption (Bug #0008)
    matcher = EventMatcher(tx_context)
    result = matcher.find_matching_pool_event(
    ...
    if result is None or result["pool_event"] is None:
        ...
        if pool_event_topic == AaveV3PoolEvent.WITHDRAW.value:
        ...
        elif pool_event_topic == AaveV3PoolEvent.LIQUIDATION_CALL.value:
...
def _process_standard_debt_burn_event(
    *,
    ...
    """Process a standard debt (vToken) burn event (non-GHO) using EventMatcher.

    Uses EventMatcher to find matching Pool events (REPAY, LIQUIDATION_CALL, or DEFICIT_CREATED)
    and handles edge cases like debt burns without matching Pool events (flash loan liquidations).
    ...
    # Use EventMatcher to find matching pool event
    # EventMatcher handles LIQUIDATION_CALL/DEFICIT_CREATED reuse (Bug #0010, #0013)
    # and REPAY conditional consumption (Bug #0008)
    matcher = EventMatcher(tx_context)
    result = matcher.find_matching_pool_event(
    ...
    # Initialize scaled_amount - will be calculated based on event type or left as None
    # for liquidations (where we use actual Burn event values)
    scaled_amount: int | None = None
    pool_event_topic: HexBytes | None = None
    ...
    if result is None or result["pool_event"] is None:
        ...
        # Extract payback amount based on matched event type
        extraction_data = result["extraction_data"]
        pool_event_topic = result["pool_event"]["topics"][0]

        if pool_event_topic == AaveV3PoolEvent.REPAY.value:
            payback_amount = extraction_data["raw_amount"]
            # EventMatcher already handles consumption based on useATokens flag
            ...
            # For liquidations, do NOT calculate scaled amount from debt_to_cover.
            # The actual burn amount in the Burn event may differ from debt_to_cover
            # due to interest accrual. Let the processor calculate from actual event values.
            # See debug report for details.
            pass
        elif pool_event_topic == AaveV3PoolEvent.DEFICIT_CREATED.value:
            payback_amount = extraction_data["amount_created"]

            # Calculate scaled amount using the original amount
            pool_processor = PoolProcessorFactory.get_pool_processor_for_token_revision(
                debt_asset.v_token_revision
            )
            scaled_amount = pool_processor.calculate_debt_burn_scaled_amount(
...
def _process_scaled_token_balance_transfer_event(
    ...
    if context.tx_context is not None:
        for prior_event in context.tx_context.get_prior_events(context.event):
            prior_topic = prior_event["topics"][0]
            if prior_topic == AaveV3ScaledTokenEvent.MINT.value:
                # Check if this Mint event has value == balanceIncrease (pure interest)
                ...
                if (
                    ...
                    # This BalanceTransfer represents the same value as the Mint
                    # Skip the balance update for the TO user
    ...
    if context.tx_context is not None:
        ...
        for subsequent_event in subsequent_events[:5]:
            ...
            if (
                subsequent_topic == AaveV3ScaledTokenEvent.BURN.value
    ...
    *,
    ...
    for event in sorted(events, key=_event_sort_key):
        ...
        logger.debug(
            ...
            f"block={block_num} tx={tx_hash.to_0x_hex()} topic={topic.to_0x_hex()} addr={event_address}"
        )
        ...
        if (
            ...
            logger.debug(
                f"_build_transaction_contexts: SKIPPING scaled token event from Pool addr={event_address}"
            )
            continue
        ...
        if topic in {
            ...
            if event_address == GHO_VARIABLE_DEBT_TOKEN_ADDRESS:
                ...
                logger.debug(
                    f"_build_transaction_contexts: categorized as DEBT_MINT event addr={event_address}"
                )
                ctx.debt_mints.append(event)
            else:
                logger.debug(
                    f"_build_transaction_contexts: categorized as COLLATERAL_MINT event addr={event_address}"
                )
            ...
            if event_address == GHO_VARIABLE_DEBT_TOKEN_ADDRESS:
                ...
                logger.debug(
                    f"_build_transaction_contexts: categorized as DEBT_BURN event addr={event_address}"
                )
                ctx.debt_burns.append(event)
            else:
                logger.debug(
                    f"_build_transaction_contexts: categorized as COLLATERAL_BURN event addr={event_address}"
                )
...
def _get_stk_aave_classifying_events(
    ...
    # Sort by priority
    priority = {
        AaveV3StkAaveEvent.STAKED.value: 0,
        AaveV3StkAaveEvent.REDEEM.value: 0,
        ERC20Event.TRANSFER.value: 1,
    }
